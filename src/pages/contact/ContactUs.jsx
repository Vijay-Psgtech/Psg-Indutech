...
import { Mail, Phone, MapPin, User, Building, Send, CheckCircle, AlertCircle } from "lucide-react";
import { motion } from "framer-motion";
import { brandColors, grad, gradText } from "../../components/common/brand.js";
import Eyebrow from "../../components/common/Eyebrow";
import usePageTitle from "../../hooks/usePageTitle.jsx";
...
const ContactPage = () => {
  ...
  const [recipientEmail, setRecipientEmail] = useState("info@psgtech.ac.in");

  // Pre-fill form from navigation state
  useEffect(() => {
    if (location.state) {
      const { recipientEmail: email, service, source } = location.state;
      ...
      // Optionally add source to message
      if (source) {
        setFormData((prev) => ({
          ...prev,
          message: `Inquiry from: ${source}\n\n`,
        }));
      }
    }
  }, [location.state]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/send-enquiry`,
        {
          method: "POST",
          headers: {
          ...
          <div className="lg:col-span-1 p-8 sm:p-10 bg-linear-to-br from-slate-50 to-white">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                >
                  <option value="">Select Service</option>
                  <option value="Laminating & Coating">Laminating & Coating</option>
                  <option value="Needle Punching Machine">Needle Punching Machine</option>
                  <option value="Thermal Wadding">Thermal Wadding</option>
                  <option value="Wipes Manufacturing">Wipes Manufacturing</option>
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="w-full py-4 rounded-xl font-bold text-white flex items-center justify-center gap-2 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                style={{
                  background: grad.subtle,
                }}
              >
