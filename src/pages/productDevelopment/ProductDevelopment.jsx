...
const ProductDevelopment = () => {
  ...
  // ✅ Navigate to contact page with product dev email
  const handleProductDevEnquiry = () => {
    navigate("/contact", {
      state: {
        recipientEmail: "pd1.int@psgtech.ac.in",
    ...
    <section className="min-h-screen w-full bg-slate-50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tight leading-tight text-slate-900 mb-6">
            Product Development <span style={gradText}>Facility</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="text-lg text-slate-600 leading-relaxed">
        ...
        >
          {productDevData.map((item, index) => (
            <motion.div
              key={index}
              variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { type: "spring", stiffness: 50 } } }}
            ...
            >
              <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-50 rounded-bl-full -mr-8 -mt-8 transition-transform group-hover:scale-150 duration-500" style={{ opacity: 0.5 }} />
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-center bg-white rounded-3xl p-4 shadow-xl border border-indigo-50 overflow-hidden">
          ...
          <motion.div className="p-6 lg:p-10 text-center lg:text-left" initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-block p-3 rounded-full bg-indigo-50 mb-6">
              <User className="w-8 h-8 text-indigo-600" />
            </div>
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-4">Need Custom Solutions?</h2>
            ...
            <div className="space-y-4">
              {/* ✅ Replaced <a href="mailto:..."> with navigate button */}
              <button
                onClick={handleProductDevEnquiry}
                className="flex items-center justify-center lg:justify-start gap-3 p-4 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 group w-full cursor-pointer"
              >
